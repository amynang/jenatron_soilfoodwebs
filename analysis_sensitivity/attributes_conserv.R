...
for (i in 1:length(att)) {
  att[[i]] = att[[i]] %>% 
    ...
    mutate(Bodymass.mg = case_when(.$taxon_name %in% nematodes ~ ecophys$AvgMass[match(.$taxon_name, ecophys$taxon_name)],
                                   # here we assume bodymass differences across units are sampling artefacts
                                   .$taxon_name %in% c(meso,macro) ~ across.unit.avg$Bodymass.mg[match(.$taxon_name,across.unit.avg$taxon_name)],
                                   TRUE ~ 1)) %>% 
